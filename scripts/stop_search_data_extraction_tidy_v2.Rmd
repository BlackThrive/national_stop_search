...
Here data on which Force serves each LAD is added. 
...
### Check for missing forces and manually add them

This section checks whether there are any LADs which do not have a force allocated, and then adds them manually

```{r}
# check which forces are missing
...
## Separate date/time into separate columns

Finally, the data/time of stop is separated from one column into two separate columns

```{r}

#data <- read_csv(file = "../data/national_dataset_with_forces.csv")

df$time <- substr(df$datetime, 12, 19)
df$date <- as.Date(substr(df$datetime, 1, 10))

df$time <- chron(times. =  df$time, format = c(times = "hh:mm:ss"))

df <- df[order(df$date,df$time),]

#write_csv(df, file = "../data/national_la_nameset_with_forces.csv")

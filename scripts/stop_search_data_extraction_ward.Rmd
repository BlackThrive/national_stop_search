...
# temp <- places %>% 
#   distinct(WD19NM, .keep_all = TRUE) %>%
#   subset(select = -c(1))

write_csv(temp, file = "../data/places_ward_to_country.csv")
...
```{r}
las <- read.csv("../data/ward_to_country_list.csv")
ogrListLayers("../data/bdline_gb.gpkg") # check layers
geo_data <- st_read("../data/bdline_gb.gpkg", layer = "district_borough_unitary_ward") # extract layer of interest
...
missing_data <- data.frame(ward_name = NA,
                           la = NA,
                           county = NA,
                           region = NA,
                           country = NA)
...
for(i in 1:length(geo_geom_wgs84)){
  ...
  try(
    geo_geom_wgs84[[i]][["la"]] <- las[which(las$WD19CD == geo_data$Census_Code[i]), "LAD19NM"]
  )
  try(
    geo_geom_wgs84[[i]][["county"]] <- las[which(las$WD19CD == geo_data$Census_Code[i]), "CTY19NM"]
  )
  try(
    geo_geom_wgs84[[i]][["region"]] <- las[which(las$WD19CD == geo_data$Census_Code[i]), "RGN19NM"]
  )
  try(
    geo_geom_wgs84[[i]][["country"]] <- las[which(las$WD19CD == geo_data$Census_Code[i]), "CTRY19NM"]
  )
  
  # report which LAs have missing values 
  if(is_empty(geo_geom_wgs84[[i]][["la"]])){
    print(paste0(names(geo_geom_wgs84)[i], " la missing (", i, ")"))
    missing_data[i, "ward_name"] <- names(geo_geom_wgs84)[i]
    missing_data[i, "la"] <- 1
  }
  if(is_empty(geo_geom_wgs84[[i]][["county"]])){
    print(paste0(names(geo_geom_wgs84)[i], " county missing (", i, ")"))
    missing_data[i, "ward_name"] <- names(geo_geom_wgs84)[i]
    missing_data[i, "county"] <- 1
  }
  if(is_empty(geo_geom_wgs84[[i]][["region"]])){
    print(paste0(names(geo_geom_wgs84)[i], " region missing (", i, ")"))
    missing_data[i, "ward_name"] <- names(geo_geom_wgs84)[i]
    missing_data[i, "region"] <- 1
  }
  if(is_empty(geo_geom_wgs84[[i]][["country"]])){
    print(paste0(names(geo_geom_wgs84)[i], " country missing (", i, ")"))
    missing_data[i, "ward_name"] <- names(geo_geom_wgs84)[i]
    missing_data[i, "country"] <- 1
    }
...
coords_for_map <- coords_unlisted[["Na h-Eileanan an Iar"]]# map wants long, lat
...
retrieve_area_data <- function(coord_list, 
  ...
  # get most recent update if most_recent data not specified
    ...
    if(is_empty(area_output)){
      if(is.na(no_entries_df[1,1])){ # if first occurence, replaces NAs 
        no_entries_df[1,] <- c(h, la)
      }
      else{ # rbind subsequent occurences
  ...
  return(list(result = overall_output,
              missing_entries = no_entries_df)) # testing... change back to overall_output
}
...
this_list <- coords_unlisted[c(123:123)]
...
```{r}
area_coords <- coords_unlisted[[123]][["coords"]]
...
for(i in 1:nrow(area_coords)){
  ...
  cat("\014")
  print(paste0(i, " of ", nrow(area_coords)))
}

save(coord_string, file = "../data/highlands_coord_string.Rdata")
date <- "2021-10"
body <- list("poly" = coord_string,
                   "date" = date)
post_request <- httr::POST("https://data.police.uk/api/stops-street?", body = body)
...
df <- httr::content(post_request) # get content from response
df_2 <- sapply(df, unlist)
df_3 <- do.call(bind_rows, df_2)
data.frame(t(sapply(df,c)))
#df_3 <- data.frame(t(sapply(lapply(df, unlist),c))) # convert to d
```
```{r}
...
```{r}
load("../data/las_1_to_50.Rdata")
data_1 <- data[[1]]
missing_areas_1 <- data[[2]]
load("../data/las_51_to_100.Rdata")
data_2 <- data[[1]]
missing_areas_2 <- data[[2]]
load("../data/las_101_to_122.Rdata")
data_3 <- data[[1]]
missing_areas_3 <- data[[2]]
load("../data/las_123.Rdata")
data_4 <- highlands_data
#missing_areas_4 <- data[[2]]
load("../data/las_124_to_150.Rdata")
data_5 <- data[[1]]
missing_areas_5 <- data[[2]]
load("../data/las_151_to_175.Rdata")
data_6 <- data[[1]]
missing_areas_6 <- data[[2]]
load("../data/las_176_to_200.Rdata")
data_7 <- data[[1]]
missing_areas_7 <- data[[2]]
load("../data/las_201_to_300.Rdata")
data_8 <- data[[1]]
missing_areas_8 <- data[[2]]
load("../data/las_301_to_363.Rdata")
data_9 <- data[[1]]
missing_areas_9 <- data[[2]]


all_data <- bind_rows(data_1,
                      data_2,
                      data_3,
                      data_4,
                      data_5,
...
all_missing_areas <- rbind(missing_areas_1,
                       missing_areas_2,
                       missing_areas_3,
                       #missing_areas_4,
                       missing_areas_5,
                       ...
                       missing_areas_8,
                       missing_areas_9)
all_missing_areas <- all_missing_areas[-c(which(is.na(all_missing_areas))),]

write_csv(all_data, file = "../data/national_stop_search_12_months_to_Oct21.csv")
```

```{r}
coords_for_map <- coords_unlisted[["Na h-Eileanan Siar"]][["coords"]]
...
issue with e.g. east ayrshire cammansnahwire

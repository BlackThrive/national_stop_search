---
title: "Lambeth Statistics Check"
...
```{r include = FALSE}
# coord_list <- coords["Lambeth"]
# lambeth_data_list <- la_search_police_api(coord_list, most_recent_month = 8, most_recent_year = 2021)
# lambeth_data_df <- lambeth_data_list[[1]]
# save(lambeth_data_df, file = "../data/lambeth_df.Rdata")

# load instead of rerunning above
load("../data/lambeth_df.Rdata")
df <- lambeth_data_df
# ordered_df <- df[order(df$datetime),]
```

# Boundary definition from which stats were retrieved

```{r}
coords_for_map <- coords[["Lambeth"]][["coords"]] # map wants long, lat
...
```{r stop_search_self}
# stop and search
data <- read.csv("../data/stop_search_lambeth_met_dashboard.csv")
...
The number for the new figures is about 10% lower than the number for the new figures for BOTH White and Black people (`r white_diff_percent`% for White,`r black_diff_percent`% for Black). Suggests that the difference is not biased by omissions of just one group.

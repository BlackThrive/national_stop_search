...
la_search_police_api_original <- function(coord_list, 
                                 most_recent_month = NULL, 
                                 most_recent_year = NULL, 
                                 num_months_backwards = 12, 
                                 wait_time = 5, 
...
test_data_original <- la_search_police_api_original(this_list, num_months_backwards = 12, most_recent_month = 10, most_recent_year = 2021)

df_original <- test_data_original[[1]]
```
...
```{r}
data <- la_search_police_api(ordered_coords, num_months_backwards = 12, most_recent_month = 10, most_recent_year = 2021)
...
```{r}
this_list <- coords[c(1:25)]
data <- la_search_police_api(this_list, num_months_backwards = 12, most_recent_month = 10, most_recent_year = 2021)
df <- data[["result"]]

unique(df$number_months_acquired)

df[which(df$number_months_acquired == 12),]
seven <-df[which(df$number_months_acquired == 7),]

five <-df[which(df$number_months_acquired == 5),]

eleven <- df[which(df$number_months_acquired == 11),]

eight <-df[which(df$number_months_acquired == 8),]
ten <-df[which(df$number_months_acquired == 10),]


bolton <- df[which(df$la == "Bolton"),]


lambeth <- df[which(df$la == "Lambeth"),]

area_coords <- this_list[["Sandwell"]][["coords"]][[1]]
coord_string <- paste0(area_coords$lat,",",area_coords$long, collapse = ":") 
sandwell_data <- temp_func(coord_string)
#write(coord_string, file = "bolton_coords.txt")
```
...
```{r}
load("../data/las_301_to_363_list.Rdata")
```
...
bolton_data <- temp_func(coord_string)
...
```{r}
area_coords <- this_list[["West Northamptonshire"]][["coords"]][[1]]
coord_string <- paste0(area_coords$lat,",",area_coords$long, collapse = ":") 
wn_data <- temp_func(coord_string, most_recent_month = 10, most_recent_year = 2021)
```
...
- separate date/time to separate columns?
...
```{r}
this_list <- coords[c(1:20)]
test_data_2 <- la_search_police_api(this_list, most_recent_month = 10, most_recent_year = 2021)
```


```{r}
sum(df$la == "West Northamptonshire")
sum(df_original$la == "West Northamptonshire")
length(unique(df$datetime))
length(unique(df_original$datetime))

wn <- subset(df, la == "West Northamptonshire")
length(unique(wn$datetime))

wn_2 <- subset(df_original, la == "West Northamptonshire")
length(unique(wn_2$datetime))
```

need to understand the above. could be that old coding was wrong? THIS NEEDS RESOLVING
...
for(i in 1:nrow(df)){
  try(
    df$force[i] <- forces_df[which(forces_df$LAD20CD == df$la_code[i]), "PFA20NM"]
  )
...
```{r}
missing_forces_df <- subset(df, force == "character(0)")

unique(missing_forces_df$la)
